...
MongoClient.connect('mongodb://127.0.0.1:27017/mixpanel', function(err, db) {
    ...
    function updateEmailedContent(event) {

        //console.log("Connected to Database");

        //update record with events
        var myCollection = db.collection('people');
        var userid = event.properties.distinct_id;
        //console.log('email event: ', event);
        myCollection.update({_id: userid}, { $inc: { emailed_content: 1 } }, function(err, records) {
            if (err) {
                throw err;
            }
        });
            ...
            if (!error && response.statusCode === 200) {
                ...
                for (var i in strLines) {
                    if(strLines[i]) {
                        ...
                        //console.log('thisJsonEvent: ', thisJsonEvent.event, 'properties: ', thisJsonEvent.properties);
                        if(thisJsonEvent.event === "Added Content" && thisJsonEvent.properties.Method === "Email") {
                            updateEmailedContent(thisJsonEvent);
                        }
